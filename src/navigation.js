...
function getRect (elem) {
  ...
  // var dataLayer = elem.getAttribute('data-layer') || 1 // the dirtiest ahck in the world here !!!
  // var verticalLayerMultiplierHack = dataLayer * 200
  var rect = {
    left: cr.left,
    top: cr.top,
    // top: cr.top - verticalLayerMultiplierHack,
    right: cr.right,
    bottom: cr.bottom,
...
const withinHorizontalDirection = (point1, point2) => {

}

const atRight = (point1, point2, trashHold = 0) => {
  return point1.left - point2.left < -trashHold
}

const atLeft = (point1, point2, trashHold = 0) => {
  return point1.left - point2.left > trashHold
}

const atTop = (point1, point2, trashHold = 0) => {
  return point1.top - point2.top > trashHold
}

const atBottom = (point1, point2, trashHold = 0) => {
  return point1.top - point2.top < -trashHold
}
...
function navigate (target, direction, candidates, config) {
  ...
  for (var i = 0; i < candidates.length; i++) {
    ...
    var layer = candidates[i].getAttribute('data-layer') || 1
    if (rect && layer === targetLayer) {
      switch (direction) {
        case 'right':
          if (atRight(targetRect, rect, minDistanceTrashHold)) {
            elements.push(rect)
          }
          break
        case 'left':
          if (atLeft(targetRect, rect, minDistanceTrashHold)) {
            elements.push(rect)
          }
          break
        case 'up':
          if (atTop(targetRect, rect, minDistanceTrashHold)) {
            elements.push(rect)
          }
          break
        case 'down':
          if (atBottom(targetRect, rect, minDistanceTrashHold)) {
            elements.push(rect)
          }
      }
    }
